...
function addMonth() {
    ...
    if (floatParsedLimit <= 0 || isNaN(floatParsedLimit)) {
        maxLimitInput.setCustomValidity("Please enter a valid number as the budget limit.");
...
function updateMainPanelStats() {
    ...
    if (month) {
        ...
        const totalExpenses = month.expenses.length === 0 ? 0 : month.expenses.reduce((a, b) => a + b.amount, 0);
        ...
        percentageBar.style.width = percentage > 100 ? "100%" : `${percentage.toFixed(2)}%`;
...
maxLimitInput.addEventListener("input", () => maxLimitInput.setCustomValidity(""));
